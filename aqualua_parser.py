...
class AqualuaParser:
    ...
    def parse_const_declaration(self) -> VariableDeclaration:
        """Parse: const name = value (no type annotation required)"""
        self.consume(TokenType.CONST)
        name = self.consume(TokenType.IDENTIFIER).value
        
        # Type annotation is optional for const
        type_annotation = None
        if self.match(TokenType.COLON):
            self.advance()
            type_annotation = self.parse_type()
    ...
    def parse_function_definition(self) -> FunctionDefinition:
        ...
        if self.match(TokenType.COLON):
            ...
            # Default to brace syntax if neither found
            use_colon_syntax = False
    ...
    def parse_indented_block(self) -> List[Statement]:
        """Parse indentation-based block with improved termination detection"""
        statements = []
        self.skip_newlines()
        ...
        while not self.match(TokenType.EOF):
            ...
            if self.match(TokenType.ELSE, TokenType.EXCEPT, TokenType.FINALLY):
                break
            
            # Stop on new top-level constructs (after first statement)
            if len(statements) > 0 and self.match(TokenType.FN, TokenType.FUNCTION, TokenType.MODEL, 
                                                  TokenType.CLASS, TokenType.IMPORT, TokenType.FROM):
                break
            
            # Stop on control structures at same level (heuristic)
            if len(statements) > 2 and self.match(TokenType.WHILE, TokenType.FOR, TokenType.IF):
                break
            ...
            if stmt:
                ...
                # If we can't parse a statement, try to skip to next line
                while not self.match(TokenType.NEWLINE, TokenType.EOF):
                    self.advance()
    ...
    def parse_try_statement(self) -> TryStatement:
        """Parse try/except/finally statement with proper error handling"""
        ...
        if use_colon_syntax:
            try_body = self.parse_indented_block()
        else:
            try_body = self.parse_brace_block()
        ...
        while self.match(TokenType.EXCEPT):
            ...
            # Handle syntax for except block
            except_colon_syntax = False
            if self.match(TokenType.COLON):
                self.advance()
                except_colon_syntax = True
            elif self.match(TokenType.LBRACE):
                except_colon_syntax = False
            else:
                except_colon_syntax = use_colon_syntax  # Match try block syntax
            
            if except_colon_syntax:
                except_body = self.parse_indented_block()
            else:
                except_body = self.parse_brace_block()
        ...
        if self.match(TokenType.FINALLY):
            ...
            # Handle syntax for finally block
            finally_colon_syntax = False
            if self.match(TokenType.COLON):
                self.advance()
                finally_colon_syntax = True
            elif self.match(TokenType.LBRACE):
                finally_colon_syntax = False
            else:
                finally_colon_syntax = use_colon_syntax  # Match try block syntax
            
            if finally_colon_syntax:
                finally_body = self.parse_indented_block()
            else:
                finally_body = self.parse_brace_block()
