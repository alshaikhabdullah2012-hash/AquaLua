...
try:
    ...
    backend = None
from aqualua_pyobject import PyObjectValue, call_cache, python_get_attr, python_call_cached, python_import_module
...
class AqualuaInterpreter:
    ...
    def setup_builtins(self):
        ...
        try:
            from aqualua_gui import has_event, wait_for_event
            self.environment.define("has_event", has_event)
            self.environment.define("wait_for_event", wait_for_event)
        except ImportError:
            pass
        ...
        def exec_func(code):
            try:
                ...
                for name, value in self.environment.variables.items():
                    if isinstance(value, PyObjectValue):
                        shared_globals[name] = value.obj
                    else:
                ...
                for name, value in shared_globals.items():
                    if not name.startswith('__') and name not in globals() and name not in builtin_names:
                        if hasattr(value, '__call__') and not isinstance(value, type):
                            # Wrap functions as PyObjects
                            self.environment.define(name, PyObjectValue(value))
        ...
        # PyObject Bridge Functions
        self.environment.define("python_import", python_import_module)
        self.environment.define("python_call", python_call_cached)
        self.environment.define("python_attr", python_get_attr)
    ...
    def execute_for_statement(self, statement: ForStatement):
        ...
        try:
            # Handle PyObject iteration
            if isinstance(iterable, PyObjectValue):
                # Convert PyObject to Python iterable
                try:
                    py_iterable = iterable.obj
                    for item in py_iterable:
                        # Wrap Python objects as PyObjectValue if needed
                        if isinstance(item, (int, float, str, bool, type(None))):
                            wrapped_item = item
                        else:
                            wrapped_item = PyObjectValue(item)
                        
                        # Define loop variable in current environment
                        self.environment.define(statement.variable, wrapped_item)
                        try:
                            for stmt in statement.body:
                                self.execute(stmt)
                        except BreakException:
                    ...
                    raise AqualuaRuntimeError(f"PyObject is not iterable: {e}")
    ...
    def execute_import_statement(self, statement: ImportStatement):
        """Execute import statement - Python objects as foreign values"""
        # Handle module_path which is a list
        if hasattr(statement, 'module_path'):
            module_path = statement.module_path
            module_name = '.'.join(module_path)
        else:
            module_name = statement.module if hasattr(statement, 'module') else 'unknown'
            module_path = [module_name]
        
        # Use alias if provided, otherwise use the last part of module path
        alias = statement.alias if hasattr(statement, 'alias') and statement.alias else module_path[-1]
        
        # Import as PyObject - zero library knowledge required
        pymodule = python_import_module(module_name)
        
        if pymodule:
            self.environment.define(alias, pymodule)
            print(f"[PYOBJECT] Imported {module_name} as foreign PyObject")
        else:
            print(f"[PYOBJECT] Failed to import {module_name}")
        
        return None
    
    def execute_from_import_statement(self, statement):
        """Execute from import statement - import specific items from module"""
        ...
        if pymodule:
            ...
            for name in statement.names:
                ...
                attr = python_get_attr(pymodule.obj, name)
                if attr:
                    # Wrap as PyObject if needed
                    if isinstance(attr, (int, float, str, bool, type(None))):
                        self.environment.define(name, attr)
                    else:
                        self.environment.define(name, PyObjectValue(attr))
                    print(f"[PYOBJECT] Imported {name} from {module_name}")
                else:
                    print(f"[PYOBJECT] Failed to import {name} from {module_name}")
        else:
            print(f"[PYOBJECT] Failed to import module {module_name}")
    ...
    def evaluate(self, expr: Expression) -> Any:
        ...
        if isinstance(expr, IntegerLiteral):
            ...
            # Handle dot notation with PyObject caching
            if '.' in expr.name:
                parts = expr.name.split('.')
                obj = self.environment.get(parts[0])
                
                # Navigate through PyObject attributes
                for part in parts[1:]:
                    if isinstance(obj, PyObjectValue):
                        attr = call_cache.get_cached_attr(obj.obj, part)
                        if attr is None:
                            return None
                        obj = PyObjectValue(attr) if not isinstance(attr, (int, float, str, bool, type(None))) else attr
                    elif obj is not None and hasattr(obj, part):
    ...
    def evaluate_function_call(self, call: FunctionCall) -> Any:
        ...
        try:
            ...
            if '.' in call.name:
                ...
                # Handle PyObject attribute chain
                for part in parts[1:-1]:
                    if isinstance(obj, PyObjectValue):
                        obj = python_get_attr(obj.obj, part)
                ...
                # PyObject method call with caching
                if isinstance(obj, PyObjectValue):
                    try:
                        callable_obj = call_cache.get_cached_callable(obj.obj, method_name)
                        result = python_call_cached(callable_obj, args)
                        return result
                    except:
                        return None
    ...
    def evaluate_index_expression(self, expr: IndexExpression) -> Any:
        ...
        # Handle PyObject indexing
        if isinstance(obj, PyObjectValue):
            try:
                if len(indices) == 1:
                    result = obj.obj[indices[0]]
                else:
                    result = obj.obj[tuple(indices)]
                
                # Wrap result if needed
                if isinstance(result, (int, float, str, bool, type(None))):
                    return result
                else:
                    return PyObjectValue(result)
            except Exception as e:
                raise AqualuaRuntimeError(f"PyObject indexing failed: {e}")
